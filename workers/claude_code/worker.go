...
type Config struct {
	...
	// 目录配置
	OriginalDataDir string // 原始ZIP保存目录: data/original/claude_code
...
// NewWorker 创建Claude Code Worker
...
func (w *Worker) processZip(zipPath string) (int, error) {
	...
	// 创建临时解压目录
	tempDir := filepath.Join(w.config.TempDir, fmt.Sprintf("claude_code-%d", time.Now().Unix()))
	...
	// 验证目录结构并获取 projects 目录路径
	projectsDir, err := ValidateClaudeCodeStructure(tempDir)
	if err != nil {
		// 结构不符合，标记为废弃
	...
	// 查找所有 JSONL 文件
	jsonlFiles, err := FindJSONLFiles(projectsDir)
	if err != nil {
...
func (w *Worker) syncToDatabase(conv *ParsedConversation) error {
	...
	// 插入对话
	err = w.db.InsertConversationTx(tx, conv.UUID, "claude_code", conv.Title, conv.Metadata, conv.CreatedAt, conv.UpdatedAt)
	if err != nil {
		tx.Rollback()
	...
	for _, msg := range conv.Messages {
		// 构建 content JSON
		contentJSON, err := BuildContentJSON(msg.ContentText)
		if err != nil {
