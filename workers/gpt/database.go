...
// InsertConversation 插入或更新对话
func (d *Database) InsertConversation(uuid, sourceType, title, metadata string, createdAt, updatedAt time.Time) error {
	query := `
	...
	`
	_, err := d.db.Exec(query, uuid, sourceType, title, metadata, createdAt, updatedAt)
	return err
}

// InsertMessage 插入或更新消息
func (d *Database) InsertMessage(uuid, conversationUUID, parentUUID string, roundIndex int,
	...
	`
	_, err := d.db.Exec(query, uuid, conversationUUID, parentUUID, roundIndex, role, contentType, content, createdAt)
	return err
}

// ConversationExists 检查对话是否已存在
func (d *Database) ConversationExists(uuid string) (bool, error) {
	var count int
	err := d.db.QueryRow("SELECT COUNT(*) FROM conversations WHERE uuid = ?", uuid).Scan(&count)
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func initDatabase(db *sql.DB) error {
	...
	possiblePaths := []string{
		...
		"../../scripts/init_database.sql",            // 从 workers/gpt 目录运行
		"../../../scripts/init_database.sql",         // 从 workers/gpt/cmd 目录运行
	}
...
func BuildContentJSON(text string, images []string) (string, error) {
	content := ContentJSON{}
