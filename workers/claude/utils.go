...
// ValidateClaudeStructure 验证解压后的目录结构
// 返回 conversations.json 文件的路径
func ValidateClaudeStructure(extractDir string) (string, error) {
	// 检查根目录下的 conversations.json
	conversationsPath := filepath.Join(extractDir, "conversations.json")
	if _, err := os.Stat(conversationsPath); err == nil {
		return conversationsPath, nil
	}

	// 检查是否有嵌套的顶层目录（例如 claude/conversations.json）
	...
	for _, entry := range entries {
		...
		nestedPath := filepath.Join(extractDir, entry.Name(), "conversations.json")
		if _, err := os.Stat(nestedPath); err == nil {
			return nestedPath, nil
		}
	}

	return "", fmt.Errorf("conversations.json not found")
}
