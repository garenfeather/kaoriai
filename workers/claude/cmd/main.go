...
import (
	...
	"cli-session-history/workers/claude"
)

func main() {
	// 解析命令行参数
	dbPath := flag.String("db", "data/conversations.db", "Database file path")
	dataDir := flag.String("data", "data/original/claude", "Original data directory")
	...
	log.Printf("Starting Claude Worker...")
	...
	// 创建Worker
	worker, err := claude.NewWorker(config)
	if err != nil {
