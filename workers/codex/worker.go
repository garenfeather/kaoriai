...
func (w *Worker) processZip(zipPath string) (int, error) {
	...
	// 创建临时解压目录
	tempDir := filepath.Join(w.config.TempDir, fmt.Sprintf("codex-%d", time.Now().Unix()))
	...
	// 验证目录结构并获取 sessions 目录路径
	sessionsDir, err := ValidateCodexStructure(tempDir)
	if err != nil {
		// 结构不符合，标记为废弃
	...
	// 查找所有 JSONL 文件
	jsonlFiles, err := FindJSONLFiles(sessionsDir)
	if err != nil {
		return 0, fmt.Errorf("find jsonl files: %w", err)
	}

	log.Printf("[%s] Found %d jsonl files", w.status.Name, len(jsonlFiles))

	// 解析并同步到数据库
	syncCount, err := w.processJSONLFiles(jsonlFiles)
	if err != nil {
		return 0, fmt.Errorf("process jsonl files: %w", err)
	}
...
// processJSONLFiles 处理所有 JSONL 文件
func (w *Worker) processJSONLFiles(jsonlFiles []string) (int, error) {
	syncCount := 0
	...
	for _, jsonlPath := range jsonlFiles {
		...
		log.Printf("[%s] Synced conversation %s (%d messages)", w.status.Name, conv.SessionID, len(conv.Messages))
		syncCount++
	}
...
func (w *Worker) syncToDatabase(conv *ParsedConversation) error {
	...
	// 确定对话的创建和更新时间（使用第一条和最后一条消息的时间）
	var createdAt, updatedAt time.Time
	if len(conv.Messages) > 0 {
		createdAt = conv.Messages[0].CreatedAt
		updatedAt = conv.Messages[len(conv.Messages)-1].CreatedAt
	} else {
		createdAt = time.Now()
		updatedAt = time.Now()
	}
	...
	// 插入对话
	err = w.db.InsertConversationTx(tx, conv.SessionID, "codex", conv.Title, "", createdAt, updatedAt)
	if err != nil {
		tx.Rollback()
	...
	for _, msg := range conv.Messages {
		...
		err = w.db.InsertMessageTx(tx, msg.UUID, conv.SessionID, msg.ParentUUID,
