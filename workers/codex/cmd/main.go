...
import (
	...
	"cli-session-history/workers/codex"
)

func main() {
	// 解析命令行参数
	dbPath := flag.String("db", "data/conversations.db", "Database file path")
	dataDir := flag.String("data", "data/original/codex", "Original data directory")
	...
	log.Printf("Starting Codex Worker...")
	...
	// 创建Worker
	worker, err := codex.NewWorker(config)
	if err != nil {
